// importa o db que você exportou no seu arquivo de config
import { db } from "./srcfirebaseConfig.js";

// importa funções do Firestore via CDN (v9+ modular)
import {
  collection,
  addDoc,
  getDocs,
...
async function login() {
  ...
  try {
    ...
    // pega o primeiro resultado (assumindo username único)
...
async function renderTable() {
  ...
  // ordena por data/hora se quiser (opcional)
  reservas.sort((a, b) => {
...
document.addEventListener("DOMContentLoaded", async () => {
  // prepara seletores de horário
  initTimeSelectors();

  // mostra tela de login por padrão ou restaura sessão
  ...
  // conecta botões do HTML (se existirem)
...
// algumas funções são chamadas a partir de atributos onclick do HTML,
// então precisamos deixá-las acessíveis via window
window.login = login;
